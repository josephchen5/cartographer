...
// 2D-SLAM的前端,把pose extrapolator和scan matching等模块链接起来,构成前端.
...
class LocalTrajectoryBuilder2D {
 public:
  ...
  {
    std::shared_ptr<const TrajectoryNode::Data> constant_data;      //一个轨迹节点的数据.
    std::vector<std::shared_ptr<const Submap2D>> insertion_submaps; //插入的submap
  };
